...
void usage(char *arg_0){
	printf("%s ver 20240803. Truong Hy\n", arg_0);
	printf("Usage:\n");
	...
	printf("write_ee_hex    : write hex string to EEPROM\n");
	printf("  from_addr=<n>  : from address\n");
	printf("  hex=<s>        : hex string\n");
	printf("write           : write file to normal memory\n");
	printf("  file=<s>       : file\n");
...
bool parse_params_search(char *cmdl_param, cl_my_params *my_params){
	...
	if(parse_param_exist(cmdl_param, "write_hex")){
		my_params->cmd = CMD_WRITE_NORMAL_HEXSTR;
		return true;
	}
	if(parse_param_exist(cmdl_param, "write_ee_hex")){
		my_params->cmd = CMD_WRITE_EE_HEXSTR;
		return true;
	}
	if(parse_param_exist(cmdl_param, "write")){
		my_params->cmd = CMD_WRITE_NORMAL;
		return true;
	}
	if(parse_param_exist(cmdl_param, "write_ee")){
		my_params->cmd = CMD_WRITE_EE;
		return true;
	}
	if(parse_param_exist(cmdl_param, "write_e")){
		my_params->cmd = CMD_WRITE_E;
		return true;
	}
	if(parse_param_exist(cmdl_param, "write_e20")){
		my_params->cmd = CMD_WRITE_E20;
		return true;
	}
